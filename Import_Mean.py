...
def process_files(file_paths):
    ...
    # --- ส่วนที่ 1: เตรียม Path และสร้างโฟลเดอร์ (เพิ่มใหม่) ---
    if len(file_paths) > 0:
        # ใช้ Path ของไฟล์แรกเป็นตัวตั้งต้น
        base_dir = os.path.dirname(file_paths[0])
        # สร้าง Path สำหรับโฟลเดอร์ใหม่
        output_folder = os.path.join(base_dir, 'Summary Mean')
        # สั่งสร้างโฟลเดอร์ (ถ้ามีอยู่แล้วก็ไม่ Error)
        os.makedirs(output_folder, exist_ok=True)

        # กำหนดชื่อไฟล์และ Path ปลายทางแบบเต็ม
        output_filename = "Final_Summary_Result.xlsx"
    ...
    for index, file_path in enumerate(file_paths):
        try:
            ...
            for idx, row in df.iterrows():
                # แปลง 3 คอลัมน์แรกเป็น string ตัวเล็กเพื่อเช็คคำว่า 'mean'
                first_few_cols = row.iloc[:3].astype(str).str.lower().tolist()
                # เช็คว่ามีคำว่า 'mean' อยู่ใน list หรือไม่ (เผื่อช่องว่างหรือ case sensitive)
                if any('mean' in str(x) for x in first_few_cols):
                    mean_row = row
                    break

            # กรณีหา 'mean' แบบเป๊ะๆ ไม่เจอ ลองหา row index 0 (เผื่อข้อมูลบรรทัดแรกคือ mean เลย)
            # หรือปรับ logic ตรงนี้ตามหน้างานจริงหากคำว่า Mean ซับซ้อนกว่านี้
    ...
    try:
        # ใช้ full_save_path ที่สร้างไว้ด้านบน
        ...
        # --- ส่วนแสดงผลที่อยู่ไฟล์ ---
        print("\n" + "=" * 60)
        print(f"✅ สำเร็จ! ไฟล์ถูกบันทึกเรียบร้อยแล้ว")
        print(f"📂 โฟลเดอร์: {output_folder}")
        print(f"📄 ชื่อไฟล์: {os.path.basename(full_save_path)}")
        print("=" * 60 + "\n")

        # เปิดโฟลเดอร์ให้อัตโนมัติ (Windows) โดยเปิดไปที่โฟลเดอร์ Summary Mean เลย
        if os.name == 'nt':
            os.startfile(output_folder)

    except PermissionError:
        print(f"\n❌ Error: ไม่สามารถบันทึกไฟล์ได้! กรุณาปิดไฟล์ {output_filename} ก่อนรันโปรแกรมใหม่")
    except Exception as e:
