...
# ==========================================
# 4. Scaling & Plotting (แก้ไข Fix แกน Z-Score)
# ==========================================
...
def plot_metric_comparison(df, filename_base, header_title, time_axis, metric_type):
    try:
        ...
        axs[0, 0].set_title(f"Min-Max (0-1) [Comparison View]", fontsize=14, color='blue')
        axs[0, 1].set_title(f"Z-Score (SD) [Comparison View]", fontsize=14, color='green')
        ...
        for i, sensor in enumerate(sensors):
            ...
            if col_name in df.columns:
                ...
                axs[i, 0].set_ylim(-0.05, 1.05)  # Fix Min-Max

                # --- Plot Z-Score (Right) ---
                axs[i, 1].plot(time_axis, zs.rolling(7, center=True, min_periods=1).mean(), color='tab:green')
                axs[i, 1].grid(True, alpha=0.3)
                axs[i, 1].axhline(0, color='black', lw=0.8, alpha=0.5)

                # *** Logic ใหม่: Fix แกน Z-Score ตามข้อมูล Global ***
                if GLOBAL_STATS and col_name in GLOBAL_STATS:
                    g_mean = GLOBAL_STATS[col_name]['mean']
                    g_std = GLOBAL_STATS[col_name]['std']
                    g_min_raw = GLOBAL_STATS[col_name]['min']
                    g_max_raw = GLOBAL_STATS[col_name]['max']

                    if g_std != 0:
                        # คำนวณขอบเขต Z-Score สูงสุดและต่ำสุดของประชากรทั้งหมด
                        z_max_global = (g_max_raw - g_mean) / g_std
                        z_min_global = (g_min_raw - g_mean) / g_std

                        # เผื่อขอบบนล่างอีกนิดหน่อย (10%) จะได้ไม่ชิดเกินไป
                        margin = (z_max_global - z_min_global) * 0.1
                        axs[i, 1].set_ylim(z_min_global - margin, z_max_global + margin)
...
# ==========================================
# 5. Helper & Main Logic (เหมือนเดิม)
# ==========================================
def parse_filename_info(filename):
    base = os.path.splitext(filename)[0]
    parts = base.split('_')
    if len(parts) < 2: return "Unknown_Scenario", "Unknown_Person", base
    try:
        id_str = parts[-1]
        name_str = parts[-2]
...
def process_single_file(path, scenario, name, file_id):
    try:
        ...
        with pd.ExcelWriter(xls_name, engine='openpyxl') as w:
            ...
            ws['A1'] = "Title:";
            ws['B1'] = full_title
            if GLOBAL_STATS: ws['A2'] = "** GLOBAL SCALING (All Axes Fixed) **"
...
def analyze_auto_mode():
    ...
    print("\n👉 กรุณาเลือกไฟล์ข้อมูล **ทั้งหมด** (Batch Mode)")
    root = tk.Tk();
    root.withdraw();
    root.attributes('-topmost', True)
    file_paths = filedialog.askopenfilenames(title="เลือกไฟล์ CSV ทั้งหมด", filetypes=[("CSV", "*.csv")])
    ...
    print(f"✅ พบข้อมูล {len(file_paths)} ไฟล์ แบ่งเป็น {total_people} คน")

    for i, (person, data_list) in enumerate(grouped_data.items(), 1):
        print(f"\n   👤 Processing Person {i}/{total_people}: {person}")
        person_output_files = []
        for idx, item in enumerate(data_list, 1):
            print(f"   [{idx}/{len(data_list)}] {item['scenario']} (ID: {item['id']})")
        ...
        if person_output_files:
            print(f"   💾 เลือกที่เก็บของ {person}...")
...
if __name__ == "__main__":
    while True:
        print("\n" + "=" * 40)
        print("   SENSOR ANALYZER (FULL FIXED AXIS)")
        print("=" * 40)
